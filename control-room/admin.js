...
function filtered(){const term=$('#search').value.trim().toLowerCase(),ff=$('#fitnessFilter').value;return users.filter(u=>{const text=`${name(u)} ${u.email||''}`.toLowerCase(),configured=!!fit(u).configured;return text.includes(term)&&(!ff||(ff==='configured'?configured:ff==='missing'?!configured:needsAttention(u)))})}
function renderUsers(){const rows=filtered();$('#shownCount').textContent=`מוצגים ${rows.length} מתוך ${users.length}`;$('#usersTable').innerHTML=rows.length?`<div class="table-wrap"><table><thead><tr><th>מצב</th><th>שם</th><th>מסך נוכחי</th><th>זמן במסך</th><th>כניסה אחרונה</th><th>נקודות</th><th>מילים</th><th>אימונים</th><th>איתור תקיעה</th><th>פעולות</th></tr></thead><tbody>${rows.map(u=>`<tr><td>${isOnline(u)?'<span class="online-dot"></span> מחובר':'לא מחובר'}</td><td><b>${esc(name(u))}</b><br><small>${esc(u.email||'')}</small></td><td>${esc(screenName(u.currentScreen))}</td><td>${isOnline(u)?screenMinutes(u)+' דק׳':'—'}</td><td>${fmt(u.lastActiveAt||u.lastSeenAt)}</td><td>${points(u)}</td><td>${known(u).length}</td><td>${logs(u).length}</td><td>${stuck(u)?`<span class="stuck">${esc(stuck(u))}</span>`:'תקין'}</td><td><div class="actions"><button class="btn small secondary edit" data-id="${esc(u.uid)}">ניהול</button><button class="btn small danger del" data-id="${esc(u.uid)}">מחיקה</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">לא נמצאו משתמשים</div>';document.querySelectorAll('.edit').forEach(b=>b.onclick=()=>openEdit(users.find(u=>u.uid===b.dataset.id)));document.querySelectorAll('.del').forEach(b=>b.onclick=()=>removeUser(users.find(u=>u.uid===b.dataset.id)))}
function openEdit(u){if(!u)return;currentUser=u;const f=fit(u),p=profile(u);$('#modalTitle').textContent=`ניהול ${name(u)}`;$('#userDetails').innerHTML=`<div class="detail-grid"><div class="detail">מצב<b>${isOnline(u)?'🟢 מחובר עכשיו':'לא מחובר'}</b></div><div class="detail">מסך נוכחי<b>${esc(screenName(u.currentScreen))} · ${screenMinutes(u)} דק׳</b></div><div class="detail">אימייל<b>${esc(u.email||'—')}</b></div><div class="detail">איתור תקיעה<b>${esc(stuck(u)||'לא זוהתה תקיעה')}</b></div></div><h3>עריכת פרטים</h3><div class="form-grid"><div class="field"><label>שם</label><input id="editName" value="${esc(name(u)==='—'?'':name(u))}"></div><div class="field"><label>גיל</label><input id="editAge" value="${esc(p.age||'')}"></div><div class="field"><label>נקודות</label><input id="editPoints" type="number" min="0" value="${points(u)}"></div><div class="field"><label>רמת כושר</label><select id="editFitness"><option value="beginner" ${f.level==='beginner'?'selected':''}>מתחיל</option><option value="intermediate" ${f.level==='intermediate'?'selected':''}>בינוני</option><option value="advanced" ${f.level==='advanced'?'selected':''}>מתקדם</option></select></div></div><button id="saveUser" class="btn" style="margin-top:14px">שמירת שינויים</button><hr><h3>הודעה אישית</h3><div class="field"><input id="personalTitle" placeholder="כותרת"></div><div class="field" style="margin-top:8px"><textarea id="personalBody" rows="4" placeholder="תוכן ההודעה"></textarea></div><button id="sendPersonal" class="btn success" style="margin-top:10px">שליחת הודעה למשתמש</button><hr><h3>פעולות מהירות</h3><div class="actions wrap"><button id="rewardPoints" class="btn secondary">הענקת נקודות</button><button id="resetFitness" class="btn danger">איפוס כושר</button><button id="resetEnglish" class="btn danger">איפוס אנגלית</button><button id="resetAll" class="btn danger">איפוס כל ההתקדמות</button></div>`;$('#saveUser').onclick=saveUser;$('#sendPersonal').onclick=sendPersonal;$('#rewardPoints').onclick=rewardPoints;$('#resetFitness').onclick=()=>resetProgress('fitness');$('#resetEnglish').onclick=()=>resetProgress('english');$('#resetAll').onclick=()=>resetProgress('all');$('#userModal').classList.remove('hidden')}
async function saveUser(){if(!currentUser)return;const s=structuredClone(state(currentUser)),f=structuredClone(fit(currentUser));s.profile={...(s.profile||{}),name:$('#editName').value.trim(),age:$('#editAge').value.trim()};s.points=Math.max(0,Number($('#editPoints').value)||0);f.level=$('#editFitness').value;await updateDoc(doc(db,'users',currentUser.uid),{displayName:$('#editName').value.trim(),state:s,fitness:f,updatedAt:serverTimestamp()});toast('השינויים נשמרו')}
async function sendPersonal(){const title=$('#personalTitle').value.trim(),body=$('#personalBody').value.trim();if(!title||!body)return alert('יש למלא כותרת ותוכן');const exp=new Date();exp.setDate(exp.getDate()+30);await addDoc(collection(db,'personalMessages'),{recipientUid:currentUser.uid,title,body,senderName:'אורי',active:true,createdAt:serverTimestamp(),expiresAt:Timestamp.fromDate(exp),createdBy:auth.currentUser.uid});toast('ההודעה האישית נשלחה מאורי');$('#personalTitle').value='';$('#personalBody').value=''}
async function rewardPoints(){const amount=Number(prompt('כמה נקודות להעניק?','20'));if(!Number.isFinite(amount)||amount<=0)return;const s=structuredClone(state(currentUser));s.points=points(currentUser)+Math.round(amount);await updateDoc(doc(db,'users',currentUser.uid),{state:s,updatedAt:serverTimestamp()});toast(`${Math.round(amount)} נקודות הוענקו`)}
async function resetProgress(type){if(!confirm('האם לבצע את האיפוס? הפעולה אינה ניתנת לביטול.'))return;const s=structuredClone(state(currentUser));let f=structuredClone(fit(currentUser));if(type==='fitness'||type==='all'){f={...f,logs:[],history:[],completed:{},points:0,activeWorkout:null};s.fitnessHistory=[]}if(type==='english'||type==='all'){s.known=[];s.knownWords=[];s.english={...(s.english||{}),known:[],level:0,wordsToday:0};s.completed={letters:false,words:false,quiz:false};s.quizHistory=[];s.mistakes=[]}if(type==='all'){s.points=0;s.studyDays={};s.achievements=[]}await updateDoc(doc(db,'users',currentUser.uid),{state:s,fitness:f,updatedAt:serverTimestamp()});toast('ההתקדמות אופסה')}
async function removeUser(u){if(!u||!confirm(`למחוק את נתוני ${name(u)} מ-Firestore?`))return;await deleteDoc(doc(db,'users',u.uid));toast('נתוני המשתמש נמחקו')}
$('#sendBroadcast').onclick=async()=>{const title=$('#broadcastTitle').value.trim(),body=$('#broadcastBody').value.trim(),days=Number($('#broadcastExpiry').value)||7;if(!title||!body)return alert('יש למלא כותרת ותוכן');const expires=new Date();expires.setDate(expires.getDate()+days);await addDoc(collection(db,'broadcasts'),{title,body,senderName:'אורי',active:true,createdAt:serverTimestamp(),expiresAt:Timestamp.fromDate(expires),createdBy:auth.currentUser?.uid||''});$('#broadcastModal').classList.add('hidden');toast('ההודעה נשלחה מאורי לכל המשתמשים')};
$('#export').onclick=()=>{const q=v=>'"'+String(v??'').replaceAll('"','""')+'"',csv=['שם,אימייל,מחובר,מסך,כניסה אחרונה,נקודות,מילים,אימונים,תקיעה',...filtered().map(u=>[name(u),u.email,isOnline(u)?'כן':'לא',screenName(u.currentScreen),dateOf(u.lastActiveAt||u.lastSeenAt)?.toISOString()||'',points(u),known(u).length,logs(u).length,stuck(u)].map(q).join(','))].join('\n'),a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv'}));a.download='mitkadmim-users.csv';a.click();URL.revokeObjectURL(a.href)};
